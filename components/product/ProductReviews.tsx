...
function SuccessMessage({ variant }: {
    variant: string;
}) {
    return (<div class="max-lg:py-6 lg:my-8 text-center w-[calc(50%-10px)]">
...
function ReviewForm({ yvkey, yvauth, product }: FormProps) {
    ...
    const handleReview: JSX.GenericEventHandler<HTMLFormElement> = async (e) => {
        ...
        const { reviewRating, reviewComment, reviewUserEmail, reviewUserName, } = formProps;
    ...
    return (<article id="formReview" class="font-quicksand block">
                ...
                <section class="py-6">
                    ...
                    <div>
                      ...
                      <div type="review" class="flex items-center justify-start rating">
                        <div class="rating relative box-border !flex gap-1">
                          <input type="radio" required name="reviewRating" value={0} class="rating-hidden !w-0 !h-0" disabled checked/>
                          {ratings.map((rating) => (<input type="radio" required id="reviewRating" name="reviewRating" value={rating} class="mask mask-star bg-secondary w-5 h-5"/>))}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="flex justify-between items-end mb-5">
                    <div class="border-0 p-0 text-xs relative w-full mb-3 text-[#111111]">
                      <label for="reviewComment" class="pb-3 block text-base leading-3 font-bold ">
                        Avaliação
                      </label>
                      <textarea id="reviewComment" required name="reviewComment" rows={4} placeholder="Compartilhe o que achou do produto" class="border border-[#707070] p-4 overflow-auto resize-y w-full text-sm"/>
                    </div>
                  </div>
                  <div class="flex justify-between items-end">
                    <div class="flex flex-wrap gap-x-5 max-w-[18.75rem] text-[#111111]">
                      ...
                      <div class="">
                        <input type="radio" required class="opacity-0 w-[13px] h-[13px] cursor-pointer absolute -z-10 overflow-hidden -m-[1px] p-0 border-0 peer" name="reviewRecommends" id="Sim" value="Sim"/>
                        <label for="Sim" class="block text-base font-bold leading-3 relative mb-[0.625rem] select-none cursor-pointer before:relative before:inline-block before:content-[''] before:align-middle before:rounded-[2px] before:border before:border-secondary before:w-5 before:h-5 before:mr-3 before:transition-all peer-checked:before:border-[#707070] after:content-[''] after:bg-secondary after:top-0 after:left-0 after:w-5 after:h-5 after:absolute after:block after:transition-all after:scale-0 peer-checked:after:scale-100">
                          Sim
                        </label>
                      </div>
                      <div class="">
                        <input type="radio" required class="opacity-0 w-[13px] h-[13px] cursor-pointer absolute -z-10 overflow-hidden -m-[1px] p-0 border-0 peer" name="reviewRecommends" id="Não" value="Não"/>
                        <label for="Não" class="block text-base font-bold leading-3 relative mb-[0.625rem] select-none cursor-pointer before:relative before:inline-block before:content-[''] before:align-middle before:rounded-[2px] before:border before:border-secondary before:w-5 before:h-5 before:mr-3 before:transition-all peer-checked:before:border-[#707070] after:content-[''] after:bg-secondary after:top-0 after:left-0 after:w-5 after:h-5 after:absolute after:block after:transition-all after:scale-0 peer-checked:after:scale-100">
                          Não
                        </label>
                      </div>
                    </div>
                ...
                <section class="py-6 border-t border-[#cecece]">
                  ...
                  <div class="mb-5 flex justify-between items-end">
                    <div class="border-0 p-0 text-sm relative">
                      <label class="pb-5 block text-base font-bold leading-3 text-[#111111]" for="reviewUserName">
                        Seu nome (como será exibido no site)
                      </label>
                      <input required id="reviewUserName" type="text" name="reviewUserName" placeholder="Seu nome" class="border border-[#707070] text-sm p-4 h-8 w-full"/>
                    </div>
                  </div>
                  <div class="flex justify-between items-end">
                    <div class="border-0 p-0 text-sm relative">
                      <label class="pb-5 block text-base font-bold leading-3 text-[#111111]" for="reviewUserEmail">
                        Seu email
                      </label>
                      <input required id="reviewUserEmail" type="email" name="reviewUserEmail" placeholder="Seu e-mail" class="border border-[#707070] text-sm p-4 h-8 w-full"/>
                    </div>
                  </div>
                </section>
                <button type="submit" class="rounded-[0.3125rem] text-base font-semibold h-12 my-4 w-[16.25rem] transition-all duration-300 flex justify-center items-center border border-transparent relative px-8 bg-primary hover:bg-primary-focus text-white appearance-none max-lg:mx-auto">
                  Enviar avaliação
                </button>
              </form>)}
        </>)}
...
function QuestionForm({ yvkey, product }: FormProps) {
    ...
    return (<article id="questionForm" class="font-quicksand block">
      {loading.value ? <Loading /> : (<>
          {success.value
                ...
                <section class="py-6">
                  ...
                  <div class="flex justify-between items-end mb-5">
                    <div class="border-0 p-0 text-xs relative w-full mb-3 text-[#111111]">
                      <label for="userQuestion" class="pb-5 block text-base leading-3 font-bold ">
                        O que você quer saber sobre este produto?
                      </label>
                      <textarea id="userQuestion" required name="userQuestion" rows={4} placeholder="Pergunte sobre as características do produto, como utilizá-lo ou peça alguma dica" class="border border-[#707070] p-4 overflow-auto resize-y w-full text-sm"/>
                    </div>
                  </div>
                  <h3 class="text-primary text-2xl leading-6 font-medium mb-6">
                    Seus Dados
...
export async function loader({ yourViews, page }: Props, _req: Request) {
    ...
    const reviews = await fetch(`https://service.yourviews.com.br/api/v2/pub/review/${inProductGroupWithID}?page=1&count=3&orderBy=1`, options).then((r) => r.json());
...
function ProductReviews({ reviews, product, yourViews }: SectionProps<typeof loader>) {
    if (!product)
        return null;
    const { inProductGroupWithID } = product;
    ...
    const [currentReviews, setCurrentReviews] = useState(Element ? Element.Reviews : []);
    ...
    return (<div id="reviews-container" class="font-quicksand">
      ...
      <div class={`lg:bg-white lg:flex lg:flex-col lg:justify-start ${loadingPageReview.value ? "opacity-30 cursor-wait" : ""}`}>
        {hasRatings && (<>
            <div class="w-full font-quicksand">
              ...
              <div class="py-8 w-full">
                ...
                const qualidade = review.CustomFields.find((customField) => customField.Name === "Qualidade");
                const qualidadeValue = qualidade
                    ? customFieldsValue[qualidade.Values[0] as keyof typeof customFieldsValue]
                    : review.Rating;
                const design = review.CustomFields.find((customField) => customField.Name === "Design");
                const designValue = design
                    ? customFieldsValue[design.Values[0] as keyof typeof customFieldsValue]
                    : review.Rating;
                const recommend = review.CustomFields.find((customField) => customField.Name ===
                    "Você recomenda esse produto?");
                const recommendValue = recommend
                    ? recommend.Values[0] === "Sim"
                    : review.Rating > 3;
                return (<li class="py-6 px-4 flex rounded-[0.3125rem] border border-[#dbdbdb] lg:flex-row max-lg:flex-col">
                  ...
                  <nav class="my-4">
                    <ul class="flex justify-center items-center p-0 m-0">
                      <li class="flex justify-center items-center border-none bg-transparent text-primary">
                        <button class="text-lg font-semibold px-[0.3125rem] h-6 min-w-[1.5rem] flex overflow-visible m-0 border-0 bg-transparent justify-center items-center appearance-none disabled:opacity-30 disabled:cursor-not-allowed" onClick={() => handlePageChange(PageNumber - 1)} title={`Ir para a página anterior`} type="button" disabled={IsFirstPage || loadingPageReview.value}>
                      ...
                      {pages.map((page) => (<li class={`flex justify-center items-center border-none bg-transparent ${page === PageNumber
                            ? "text-secondary underline pointer-events-none"
                            : "text-primary"}`}>
                          <button class="text-lg font-semibold px-[0.3125rem] h-6 min-w-[1.5rem] flex overflow-visible m-0 border-0 bg-transparent justify-center items-center appearance-none" onClick={() => handlePageChange(page)} title={`Ir para a página ${page}`} type="button" disabled={loadingPageReview.value}>
                            {page}
                          </button>
                        </li>))}
                      <li class="flex justify-center items-center border-none bg-transparent text-primary">
                        <button class="text-lg font-semibold px-[0.3125rem] h-6 min-w-[1.5rem] flex overflow-visible m-0 border-0 bg-transparent justify-center items-center appearance-none disabled:opacity-30 disabled:cursor-not-allowed" onClick={() => handlePageChange(PageNumber + 1)} title={`Ir para a próxima página`} type="button" disabled={IsLastPage || loadingPageReview.value}>
                          <div class="-rotate-90 relative inline-block h-[0.625rem] w-[0.625rem]">
                            <span class="left-0 rotate-45 top-[0.3125rem] absolute w-[0.384375rem] h-[0.05rem] bg-primary inline-block transition-all">
                            </span>
                            <span class="right-0 -rotate-45 top-[0.3125rem] absolute w-[0.384375rem] h-[0.05rem] bg-primary inline-block transition-all">
      ...
      <p class="text-[#707070] text-xl font-medium leading-[1.5625rem] py-5">
        {`Possui esse produto? ${hasRatings ? "Avalie-o!" : "Seja o primeiro a avaliá-lo!"}`}
      </p>
      <div class="flex justify-start lg:flex-row flex-col lg:gap-6">
        <div id="formReviewButton" class="max-lg:py-4 max-lg:mx-auto">
          <button class="hover:bg-primary-focus bg-primary text-white rounded-[0.3125rem] text-base font-medium h-12 w-[17.5rem] transition-all duration-300 max-lg:mx-auto" onClick={openReviewForm}>
            Fazer avaliação
          </button>
        </div>
        <div id="questionFormButton" class="max-lg:py-4 max-lg:mx-auto">
          <button class="hover:bg-primary-focus bg-primary text-white rounded-[0.3125rem] text-base font-medium h-12 w-[17.5rem] transition-all duration-300 max-lg:mx-auto" onClick={openQuestionForm}>
            Escrever dúvida
          </button>
        </div>
      </div>
      {displayReviewForm.value && (<ReviewForm yvkey={yourViews.key} yvauth={yourViews.auth} product={product}/>)}
      {displayQuestionForm.value && (<QuestionForm yvkey={yourViews.key} product={product}/>)}
    </div>);
}
